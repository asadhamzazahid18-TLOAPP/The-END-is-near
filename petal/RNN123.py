...
# Replace the two LSTM layers with these:
...
# ══════════════════════════════════════════════════════════════════
print("\n" + "═" * 52)
print("  OpenAI — RNN Revenue Prediction Results (Monthly)")
print("═" * 52)
for name, val, fmt in [
    ("RMSE",            rmse,    ".4f"),
    ("MAE",             mae,     ".4f"),
...
axes[0].set_title('OpenAI — RNN: Actual vs Predicted (Monthly)')
...
fig2.suptitle("OpenAI RNN Revenue Model — Performance Summary",
              color='white', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig("RNN_metrics_summary.png", dpi=150,
            bbox_inches='tight', facecolor=BG)
plt.show()
plt.close()
print("Saved → Rnn_openai.png   lstm_metrics_summary.png")

results = {
    "model": "RNN", "company": "OpenAI", "frequency": "monthly",
...
with open('rnn_metrics2.json', 'w') as f:
    json.dump(results, f, indent=2)
print("Saved → rnn_metrics2.json")

# Note: Results are based on synthetic data; real-world RNN performance may vary significantly.
